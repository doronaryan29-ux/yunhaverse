...
const SnsUpdater = () => {
  ...
  const requesterRole = isCreativeRole(user?.role) ? 'sns' : profileRole || 'sns'
  const stage = 'sns'
  const contextStage = 'copywriter'
        ...
        <section className="mt-8 flex-1 space-y-8 lg:mt-0">
          <header className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              Publish-ready assets and captions routed here.
              </p>
            </div>
            ...
            <div className="mt-6 grid gap-4">
              {assignedRequests.length === 0 ? (
                <div className="rounded-2xl border border-rose-100 bg-rose-50/70 p-4 text-sm text-slate-600">
                  No assignments yet. Stay ready for new publishing briefs.
                </div>
              ) : (
